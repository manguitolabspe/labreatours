        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-16">
          <div className="max-w-sm">
            ...
            <p className="text-gray-500 text-sm leading-relaxed mb-8 italic">
              "Fomentamos contenido basado en promoción y orientación turística del Distrito La Brea. Tu aventura comienza en el punto más occidental de Sudamérica."
            </p>
            <SocialLinks 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 flex-grow justify-end w-full md:w-auto">
            <div>
              <h5 className="text-[10px] uppercase tracking-[0.3em] font-bold text-sky-400 mb-8 underline decoration-sky-400/30 underline-offset-8">Mapa del Sitio</h5>
              <ul className="space-y-4 text-sm text-gray-400 font-medium">
                {NAV_ITEMS.map(item => (
                  <li key={item.id}>
                    <button onClick={() => onNavigate(item.id)} className="hover:text-white transition-colors uppercase tracking-widest text-[10px]">{item.label}</button>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h5 className="text-[10px] uppercase tracking-[0.3em] font-bold text-sky-400 mb-8 underline decoration-sky-400/30 underline-offset-8">Contacto Directo</h5>
